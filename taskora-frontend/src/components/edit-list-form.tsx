...
export function EditListForm(props: EditListFormnProps) {
    ...
    const updateList = () => {
        if(/\S/.test(title ?? '') && title != 'Basket' && title != 'Completed') {
            if(taskManagerContext.state.user !== undefined) {
                const UpdateListDTO: UpdateListDTO = {title: title, sections: props.list.sections, icon: icon, color: color, deleted: false}
