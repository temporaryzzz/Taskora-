			...
			return (
				<div className="task-list">
					...
					<ul className="task-list__section" id="inbox-list">
						<SortedTasks
							tasks={taskManagerContext.tasks?.filter((task) => task.completed === false)}
						/>
					</ul>

					<span
						className="task-list__title"
						onClick={() => changeVisibleList(taskListCompletedRef)}>
						<h5 style={{ float: 'left', fontWeight: 700 }}>Выполнено</h5>
					</span>

					<ul
						className="task-list__section task-list__section--completed"
						ref={taskListCompletedRef}
						id="completed-list">
						<SortedTasks
							tasks={taskManagerContext.tasks?.filter((task) => task.completed === true)}
						/>
					</ul>
				</div>
			);
