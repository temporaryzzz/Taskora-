...
function SingInForm({
		...
		try {
			if (/@/.test(username)) {
				fetch(`${SERVER_ADDRES}/auth/signin`, {
					...
					.then((data) => {
						setUser({ username: data.username, user_id: data.id, email: data.email });
	...
	return (
		<>
			<h1>TASKORA</h1>
			<form id="singin-form" action="/profile" data-js-singin onSubmit={handleSubmit}>
				<p className="field">
			...
			<form>
				<p>No account?</p>
				<button className="main-button" type="submit" formAction={'sing-up'}>
					Sing-up
				</button>
			</form>
		</>
