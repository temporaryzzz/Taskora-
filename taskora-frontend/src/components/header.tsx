...
function Header(props: HeaderProps) {
  ...
  const setActiveTab = (active: string) => {
    if (profileRef.current)
      profileRef.current.classList.remove('tabs__tab--active');
    if (taskListsRef.current)
      taskListsRef.current.classList.remove('tabs__tab--active');
    if (taskBoardRef.current)
      taskBoardRef.current.classList.remove('tabs__tab--active');

    // Устанавливаем активный класс для нужной кнопки
    switch (active) {
      case 'profile':
        if (profileRef.current)
          profileRef.current.classList.add('tabs__tab--active');
        break;
      case 'task-lists':
        if (taskListsRef.current)
          taskListsRef.current.classList.add('tabs__tab--active');
        break;
      case 'task-board':
        if (taskBoardRef.current)
          taskBoardRef.current.classList.add('tabs__tab--active');
        break;
      default:
