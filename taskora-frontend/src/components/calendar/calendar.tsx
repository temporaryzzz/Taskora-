...
const RenderDate = (props: RenderDateProps) => {
	const { dates, week, currentDate, targetDate, setTargetDate, ChangeCurrentMonth } =
		props;
	...
	return dates.map((date) => {
		...
		if (week == 1 && date > 7) {
			if (
				...
				currentDate.month == todayDate.getMonth() + 1 &&
				currentDate.year == todayDate.getFullYear()
			) {
				return (
					<DateButton
						date={date}
						elementClass={'calendar-wrapper__date calendar-wrapper__date--today'}
						currentDate={{ year: currentDate.year, month: currentDate.month - 1 }}
			...
			if (
				...
				currentDate.year == targetDate?.getFullYear() &&
				currentDate.month - 1 == targetDate?.getMonth()
			) {
				return (
					<DateButton
						date={date}
						elementClass={'calendar-wrapper__date calendar-wrapper__date--target'}
						currentDate={{ year: currentDate.year, month: currentDate.month - 1 }}
						targetDate={targetDate}
						setTargetDate={setTargetDate}
						ChangeCurrentMonth={ChangeCurrentMonth}
				...
				return (
					<DateButton
						date={date}
						elementClass={'calendar-wrapper__date calendar-wrapper__date--not-included'}
						currentDate={{ year: currentDate.year, month: currentDate.month - 1 }}
						targetDate={targetDate}
						setTargetDate={setTargetDate}
						ChangeCurrentMonth={ChangeCurrentMonth}
		...
		if (
			date == targetDate?.getDate() &&
			currentDate.year == targetDate?.getFullYear() &&
			currentDate.month == targetDate?.getMonth()
		) {
			return (
				<DateButton
					date={date}
			...
			if (
				...
				return (
					<DateButton
						date={date}
						elementClass={'calendar-wrapper__date'}
						currentDate={currentDate}
						targetDate={targetDate}
						setTargetDate={setTargetDate}
						ChangeCurrentMonth={ChangeCurrentMonth}
...
const RenderTimeBtn = ({
	...
	ChangeTime(time: { hours: number; minutes: number }): void;
	...
	return preliminaryTime.map((time) => {
		const splitTime = time.split('');
		const newHours =
			splitTime[0] == '0' ? Number(splitTime[1]) : Number(splitTime[0] + splitTime[1]);
		const newMinutes =
			splitTime[3] == '0' ? Number(splitTime[4]) : Number(splitTime[3] + splitTime[4]);
		const newTime = { hours: newHours, minutes: newMinutes };
		if (currentTime !== time) {
			return (
				<button
					onClick={() => ChangeTime(newTime)}
					className="dropdown-menu__option-btn"
					id="calendar"
					key={time}>
					{time}
				</button>
			...
			return (
				<button
					onClick={() => ChangeTime(newTime)}
					className="dropdown-menu__option-btn dropdown-menu__option-btn--current"
					id="calendar"
					key={time}>
					{time}
				</button>
...
function Calendar() {
	...
	const ChangeColorDeadline = () => {
		if (taskDate && new Date(taskDate) < new Date()) {
			...
			if (
				taskInfoWindowDateRef?.current?.classList.contains(
					'task-info-window__date--expired'
				)
			) {
	...
	const InizializateTableDates = () => {
		...
		for (let i = 1; i < 43; i++) {
			...
			if (firstDayCurrentMonth == 0) {
				...
				newDates.push(
					new Date(currentYear, currentMonth, i + 1 - firstDayCurrentMonth).getDate()
				);
			}
		}
		setDates(newDates);
	...
	const ChangeTime = (time: { hours: number; minutes: number }) => {
		...
		if (targetDate && targetDate.getHours() < 10) {
			...
			if (targetDate) hours = String(targetDate.getHours());
		}

		if (targetDate && targetDate.getMinutes() < 10) {
			minutes = '0' + targetDate.getMinutes();
		} else {
			if (targetDate) minutes = String(targetDate.getMinutes());
		}
	...
	const Cleaner = () => {
		if (taskInfo?.currentTaskInfo) {
			taskInfo.changeCurrentTask(
				taskInfo.currentTaskInfo.title,
				taskInfo.currentTaskInfo.description,
				'',
				taskInfo.currentTaskInfo.priority,
				taskInfo.currentTaskInfo.completed
			);
		}
	};
	...
	const ClickOut = (e: MouseEvent) => {
		if (e.target instanceof HTMLElement) {
			if (
				...
				!e.target.classList.contains('task-info-window__date') &&
				calendarRef?.current?.style.display == 'flex'
			) {
				if (calendarRef.current) {
					calendarRef.current.style.display = 'none';
	...
	useEffect(
		() => setFirstDayCurrentMonth(new Date(currentYear, currentMonth, 1).getDay()),
		[currentMonth]
	);
		...
		<div>
			<div
				...
				<div className="calendar-wrapper__header" id="calendar">
					<button
						className="calendar-wrapper__month-button"
						id="calendar"
						onClick={() => ChangeCurrentMonth('-')}></button>
					<div className="calendar-wrapper__month" id="calendar">
						<p>
							{monthState[currentMonth + 12]} {currentYear}
						</p>
					</div>
					<button
						className="calendar-wrapper__month-button"
				...
				<ul className="calendar-wrapper__container" id="calendar">
					<li className="calendar-wrapper__day" id="calendar">
						Пн
					</li>
					<li className="calendar-wrapper__day" id="calendar">
						Вт
				...
				<ul
					className="calendar-wrapper__container calendar-wrapper__container--weeks"
					id="calendar">
					<ul className="calendar-wrapper__container" id="calendar">
						<RenderDate
				...
				<div
					className="dropdown-menu dropdown-menu--time dropdown-menu--with-after"
					id="calendar"
					ref={dropdownMenuRef}>
					<button
						className="dropdown-menu__select-btn"
						onClick={ChangeVisibleDropdownMenu}
						id="calendar">
						<img
							src="/watch-icon.svg"
							width={'18px'}
				...
				<button className="calendar-wrapper__btn" id="calendar" onClick={Cleaner}>
					Очистить
				</button>
				<button
					className="calendar-wrapper__btn calendar-wrapper__btn--blue"
					id="calendar"
					onMouseDown={SendDate}>
					Ок
				</button>
			</div>
