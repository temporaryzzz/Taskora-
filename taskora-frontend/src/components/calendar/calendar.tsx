      ...
      if (
        ...
        return (
          <DateButton
            date={date}
            elementClass={
              'calendar-wrapper__date calendar-wrapper__date--today'
            }
            currentDate={{
      ...
      if (
        ...
        return (
          <DateButton
            date={date}
            elementClass={
              'calendar-wrapper__date calendar-wrapper__date--target'
            }
            currentDate={{
              year: currentDate.year,
              month: currentDate.month - 1,
        ...
        return (
          <DateButton
            date={date}
            elementClass={
              'calendar-wrapper__date calendar-wrapper__date--not-included'
            }
            currentDate={{
              year: currentDate.year,
              month: currentDate.month - 1,
...
const RenderTimeBtn = ({
  ...
  return preliminaryTime.map(time => {
    ...
    const newHours =
      splitTime[0] == '0'
        ? Number(splitTime[1])
        : Number(splitTime[0] + splitTime[1]);
    const newMinutes =
      splitTime[3] == '0'
        ? Number(splitTime[4])
        : Number(splitTime[3] + splitTime[4]);
...
function Calendar() {
  ...
  const ChangeColorDeadline = () => {
    if (taskDate && new Date(taskDate) < new Date()) {
      taskInfoWindowDateRef?.current?.classList.add(
      ...
      if (
        ...
        taskInfoWindowDateRef.current.classList.remove(
          'task-info-window__date--expired'
        );
      }
    }
  ...
  const ChangeVisibleDropdownMenu = () => {
    if (dropdownMenuContentRef.current && dropdownMenuRef.current) {
      if (dropdownMenuContentRef.current.style.display == 'none') {
        ...
        if (
          dropdownMenuRef.current.classList.contains('dropdown-menu--active')
        ) {
  ...
  return (
    <div>
      ...
      >
        ...
        <button
          className="calendar-wrapper__btn"
          id="calendar"
          onClick={Cleaner}
        >
          Очистить
